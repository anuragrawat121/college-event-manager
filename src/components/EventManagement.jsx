...
export function EventManagement({ darkMode, userRole, userName, events, setEvents }) {
    ...
    const [formData, setFormData] = useState({ title: "", date: "", time: "", venue: "", organizer: "", description: "", status: "upcoming" });
    const [loading, setLoading] = useState(false);

    const filteredEvents = events.filter((event) => {
        // Allow both Admin and Organizer to see all events
        const matchesRole = (userRole === 'admin' || userRole === 'organizer') ? true : event.organizer === userName;
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (selectedEvent) {
                // Edit logic (Not fully implemented in backend yet, so just local update for now or add API endpoint)
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div><h2 className={`text-2xl font-bold ${darkMode ? "text-gray-100" : "text-[#111827]"} `}>{userRole === 'admin' || userRole === 'organizer' ? "Event Management" : "My Assigned Events"}</h2><p className={`${darkMode ? "text-gray-400" : "text-gray-500"} `}>{userRole === 'admin' || userRole === 'organizer' ? "Manage all college events" : "View details of events assigned to you"}</p></div>
                {userRole === 'admin' && <button onClick={() => { setSelectedEvent(null); setFormData({ title: "", date: "", time: "", venue: "", organizer: userName, description: "", status: "upcoming" }); setIsDialogOpen(true); }} className="bg-[#3B82F6] hover:bg-[#2563EB] text-white px-4 py-2 rounded-lg flex items-center gap-2 font-medium transition-colors shadow-sm"><Plus size={18} /> Add Event</button>}
            </div>
