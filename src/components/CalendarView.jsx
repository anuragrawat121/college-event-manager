                ...
                <div key={day} className={`h-32 border-b border-r p-2 transition-colors hover:bg-opacity-50 ${darkMode ? 'border-gray-700 hover:bg-gray-800' : 'border-gray-100 hover:bg-gray-50'}`}>
                    ...
                    <div className="mt-2 space-y-1">
                        {dayEvents.map((event, idx) => (
                            <div key={idx} className={`px-2 py-1 rounded text-xs font-medium text-white truncate cursor-pointer hover:opacity-80 ${event.color || 'bg-blue-500'}`} title={event.title}>
                                {event.title}
                            </div>
                        ))}
                    </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className={`flex items-center gap-4 p-1 rounded-lg border shadow-sm ${darkMode ? 'bg-[#1E293B] border-gray-700' : 'bg-white border-gray-200'}`}>
                    <button onClick={prevMonth} className={`p-2 rounded-md transition-colors ${darkMode ? 'hover:bg-gray-700 text-gray-300' : 'hover:bg-gray-100 text-gray-600'}`}>
                        <ChevronLeft size={20} />
                    </button>
                    <span className={`font-bold px-4 min-w-[140px] text-center ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                        {monthName}
                    </span>
                    <button onClick={nextMonth} className={`p-2 rounded-md transition-colors ${darkMode ? 'hover:bg-gray-700 text-gray-300' : 'hover:bg-gray-100 text-gray-600'}`}>
