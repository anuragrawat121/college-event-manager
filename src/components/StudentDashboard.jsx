...
export function StudentDashboard({
  ...
  const getRegistrationStatus = (eventId) => {
    const reg = registrations.find(
      (r) => r.eventId === eventId && r.student === userName,
    );
  ...
  const handleConfirmRegister = async () => {
    ...
    // 1. Prepare Payload for Backend (snake_case)
    const payload = {
      event_id: confirmingEvent.id,
      student_name: userName,
      student_email: `${userName.toLowerCase()}@student.edu`,
      status: "pending",
    };
    ...
    try {
      ...
      // 3. Update Local State (Optimistic or Response based)
      // We map it back to frontend structure if needed, or just refetch.
      // For now, matching the "registrations" state format used in this component:
      // This component seems to use 'student', 'event', 'status' etc.
      const newReg = {
  ...
  return (
    <div className="space-y-8 relative">
      <div>
        <h2
          className={`text-3xl font-bold mb-2 ${darkMode ? "text-gray-100" : "text-[#111827]"}`}
        >
          Upcoming Events
        </h2>
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-start justify-between mb-2">
                  <h3
                    className={`text-lg font-bold ${darkMode ? "text-gray-100" : "text-slate-800"}`}
                  >
                    {event.title}
                  </h3>
                  <span
                ...
                {isRegistered ? (
                  <button
                    disabled
                    className="w-full bg-green-600 text-white font-medium py-2 rounded-lg flex items-center justify-center gap-2 cursor-default opacity-90"
                  >
                  ...
                  <button
                    disabled
                    className="w-full bg-yellow-100 text-yellow-700 font-medium py-2 rounded-lg flex items-center justify-center gap-2 cursor-default opacity-90"
                  >
      ...
      <div
        className={`rounded-xl border p-6 ${darkMode ? "bg-[#1E293B] border-gray-700" : "bg-white border-gray-200"}`}
      >
        <h3
          className={`text-xl font-bold mb-6 ${darkMode ? "text-gray-100" : "text-gray-800"}`}
        >
          ...
          {myEventsList.length > 0 ? (
            myEventsList.map((reg) => (
              ...
              >
                <div>
                  <h4
                    className={`font-semibold ${darkMode ? "text-gray-100" : "text-[#111827]"}`}
                  >
                    {reg.event}
                  </h4>
                  <p
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold capitalize ${getStatusColor(reg.status)}`}
                  >
                    {reg.status}
                  </span>
                  ...
                  {(reg.status === "registered" ||
                    reg.status === "attended") && (
                    <button
                      onClick={() => setTicketEvent(reg)}
                      className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors text-blue-500"
                      title="View Ticket"
                    >
                      <QrCode size={20} />
                    </button>
      ...
      {confirmingEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div
            className={`w-full max-w-sm rounded-2xl shadow-2xl p-6 ${darkMode ? "bg-[#1E293B] text-white" : "bg-white text-slate-900"}`}
          >
            <h3 className="text-xl font-bold mb-4">Confirm Registration</h3>
            <p
              className={`mb-6 ${darkMode ? "text-gray-300" : "text-gray-600"}`}
            >
              Register for <strong>{confirmingEvent.title}</strong>?
            </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl relative">
            <button
              ...
              className="absolute top-4 right-4 p-1 bg-black/10 hover:bg-black/20 rounded-full transition z-10"
            >
