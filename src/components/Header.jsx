  ...
  return (
    <header className={`h-16 px-4 md:px-8 flex items-center justify-between border-b transition-colors duration-200 relative z-20 ${darkMode ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-800'
      ...
      <div className="flex items-center gap-3 md:gap-0">
        ...
        <button
          onClick={onMenuClick}
          className="p-2 -ml-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 md:hidden text-slate-500 dark:text-slate-400"
        >
          <Menu size={24} />
        </button>
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className={`p-2 rounded-full transition-colors relative ${darkMode ? 'hover:bg-slate-800 text-slate-400' : 'hover:bg-slate-100 text-slate-600'
          ...
          {isNotifOpen && (
            <div className={`absolute right-0 mt-3 w-80 rounded-xl shadow-2xl border py-2 overflow-hidden z-50 ${darkMode ? 'bg-[#1E293B] border-gray-700 text-white' : 'bg-white border-gray-100 text-slate-800'
              ...
              <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                ...
                <span className="text-xs text-blue-500 font-medium">3 New</span>
              </div>
              <div className="max-h-64 overflow-y-auto">
                {recentNotifications.map((notif) => (
                  <div key={notif.id} className={`px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-start gap-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer`}>
                    <div className={`p-1.5 rounded-full shrink-0 ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
              ...
              <button onClick={() => { setIsNotifOpen(false); onNavigate('notifications'); }} className="w-full text-center py-2.5 text-sm text-blue-500 hover:bg-gray-50 dark:hover:bg-gray-800 font-medium">
                View all
              </button>
            </div>
          )}
        ...
        <div className="relative ml-1 md:ml-2" ref={profileRef}>
          <button
            ...
            className={`flex items-center gap-3 pl-2 md:pl-4 md:border-l transition-all ${darkMode ? 'border-slate-700 hover:opacity-80' : 'border-slate-200 hover:opacity-80'
              }`}
          >
            ...
            <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-blue-600 flex items-center justify-center text-white shadow-lg">
          ...
          {isProfileOpen && (
            <div className={`absolute right-0 mt-3 w-56 rounded-xl shadow-2xl border py-2 z-50 ${darkMode ? 'bg-[#1E293B] border-gray-700 text-white' : 'bg-white border-gray-100 text-slate-800'
              ...
              <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 mb-1 md:hidden">
                <p className="text-sm font-medium">Signed in as</p>
                <p className="text-sm font-bold truncate">{userName}</p>
              </div>
              <button onClick={() => { setIsProfileOpen(false); onNavigate('settings'); }} className="w-full text-left px-4 py-2.5 flex items-center gap-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700">
              ...
              <button onClick={onLogout} className="w-full text-left px-4 py-2.5 flex items-center gap-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20">
