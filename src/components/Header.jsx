    ...
    <header className={`h-16 px-4 md:px-8 flex items-center justify-between border-b transition-colors duration-200 relative z-20 ${darkMode ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-800'}`}>
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <div className="relative" ref={notifRef}>
          <button onClick={() => { setIsNotifOpen(!isNotifOpen); setHasUnread(false); }} className={`p-2 rounded-full transition-colors relative ${darkMode ? 'hover:bg-slate-800 text-slate-400' : 'hover:bg-slate-100 text-slate-600'} ${isNotifOpen ? 'bg-slate-100 dark:bg-slate-800' : ''}`}>
            <Bell size={20} />
          ...
          {isNotifOpen && (
            <div className={`absolute right-0 mt-3 w-80 rounded-xl shadow-2xl border py-2 overflow-hidden z-50 ${darkMode ? 'bg-[#1E293B] border-gray-700 text-white' : 'bg-white border-gray-100 text-slate-800'}`}>
              <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center"><h3 className="font-semibold text-sm">Notifications</h3><span className="text-xs text-blue-500 font-medium">3 New</span></div>
