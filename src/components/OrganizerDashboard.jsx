        ...
        <div className="space-y-6">
            ...
            <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-[#1E293B] border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`${darkMode ? 'text-gray-400' : 'text-gray-500'} mt-1`}>
                    You have <strong>{myAssignedEvents.length}</strong> events assigned to you.
                </p>
            </div>
                    ...
                    <div key={event.id} className={`rounded-xl border overflow-hidden ${darkMode ? 'bg-[#1E293B] border-gray-700' : 'bg-white border-gray-200'}`}>
                        ...
                        <div className="p-6">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${event.status === 'Active Now' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                                    }`}>
                                    {event.status}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-4 p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{event.venue}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Users size={16} className="text-gray-400" />
                                    <span className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{event.registered} Students</span>
                                </div>
                            </div>
                            ...
                            <div className="mt-6 flex gap-3">
                                <button
                                    onClick={() => onNavigate('registrations')} // Redirects to Registrations
                                ...
                                {event.status === 'Active Now' && (
                                    <button
                                        onClick={() => onNavigate('attendance')} // Redirects to Attendance
                                        className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2"
                                    >
